...
- **Real role-based access control.** Only `owner|admin|member` strings exist and **no shipped code checks them**. The role model, per-user permission overrides, and field-level financial restrictions are entirely net-new (module 4.23). _[SUPERSEDED: the "8-role" model became **6 stored roles / 5 active** per commit P4-roles + migration `0021_role_rename_to_user` — photographer/contractor/editor merged into `user`, `client_limited` → `client`. See `src/modules/rbac/types.ts` + `docs/PIVOTS_LEDGER.md` §3.]_
