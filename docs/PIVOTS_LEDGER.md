...
| --------------------------------------------------------------------------------------------------------------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------- |
| Assignment-scoped RLS overlay on `contacts` / `projects` / `tasks` (photographer/contractor/editor sees only assigned rows) | invoices module commit   | `projects/README.md` "What's deferred"; `contacts/README.md` "RLS"; `tasks/README.md` "What's deferred" |
| `lookupExtendedMemberRole` helper + widening `OrgContext.role` to the 8-role enum                                           | invoices module commit   | `rbac/README.md` "What's deferred"                                                                      |
...
| Cross-org safety on `instantiateProjectFromTemplate` (defense in depth: orgAction wrapper + explicit org filter + RLS WITH CHECK) — proven by 3 negative tests | (this commit)                                 | `tests/integration/project-instantiation-rls.test.ts` |
