...
VIEWS: **[UPDATED — LOCKED 07-08, see `docs/decisions-since-may-docs.md` §18.4]** final set = **List, Board, Timeline/Gantt** (3 core, NOT 15). **Table CUT** (too similar to List, which already does the job with phase grouping + gating). Calendar is its OWN thing (upcoming tasks + who's assigned), not a task-view tab. Not kanban-only, not thirty. **Timeline/Gantt is IN** (reverses the earlier "no Gantt" call, §18.1): tasks as bars showing overlap + dependencies, at BOTH project level (all sub-projects on one shared timeline) AND sub-project level. **Dependency DATE-CASCADE** (§18.2): moving a date auto-reschedules dependents; a manual date override pins that task (stops its cascade). Critical-path bottleneck analysis = skip/lightweight-only, deferred (§18.3). Approved PM wireframes (linear-vs-branching tree, 3-view switcher, project-vs-sub-project timeline) are the working design direction for P6 (§18.5).
