...
11. **PII + payments baseline:** MFA, session expiry reconciled, password **min 8 + composition (≥1 uppercase / ≥1 number / ≥1 special), requirements shown in the UI** — enforced **client-side AND server-side** via `src/modules/auth/password-policy.ts` (the Better Auth `before` hook rejects non-composition on sign-up/change/reset). **HIBP breach screening WIRED** (`haveIbeenPwned` plugin, k-anonymity, no key — the load-bearing control). Composition is competitor-parity, a deliberate deviation from NIST — see `docs/decisions-2026-07-16.md` → Passwords, revised 2026-07-19. Remaining hardening tracked in `docs/pre-events-punchlist.md`.
...
5. **Every state-changing action calls `audit()`.** The action context already carries actor, IP, user agent — pass `ctx` straight through. Statically enforced by `scripts/check-actions.mjs` (run in `pnpm verify --tier=1`): every action in `actions.ts` must call `audit()` or be listed in that script's `AUDIT_EXEMPT` registry with a reason (read-only, or audits via a helper, or a documented non-audit). A genuinely read-only action belongs in `queries.ts`, not `actions.ts`.
