...
- **Per-table nuance (must handle, not skip):**
  ...
  - `files`: mixed access — authenticated proxy (`/api/files/[id]`, session+activeOrg), public share-link download (by token, no session), blob-upload callback (token, no session). Add org policy + FORCE; make the authenticated proxy use `app_authenticated`+GUC (enforced); the public/token + upload-callback paths run deliberately as the BYPASSRLS owner with org resolved from the token/payload — **document WHY in-code** at each such call site.
  - `file_share_links`, `file_share_link_events`: genuinely public-by-token by design. Add org-scoped RLS+FORCE for the authenticated/admin paths; the public token path runs as owner with org from the resolved link row — **document WHY in-code**.
  - `file_scan_diagnostics`: temporary diagnostic, **nullable** `org_id`. Null-org rows can't be org-scoped. Prefer making `org_id` NOT NULL + org policy; if null rows are structurally required, document the exact reason in-code and scope reads app-side explicitly. (This table is a candidate to drop if unused — confirm before investing.)
