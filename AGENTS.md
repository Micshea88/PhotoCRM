...
11. **PII + payments baseline:** MFA, session expiry reconciled, password **min 8 + composition (≥1 uppercase / ≥1 number / ≥1 special), requirements shown in the UI** (enforced client-side by `src/modules/auth/password-policy.ts` on sign-up/reset/account — server-side composition still to add), **HIBP breach screening = NOT yet wired** (TODO H13; it is the _load-bearing_ control once built). Composition is competitor-parity, a deliberate deviation from NIST — see `docs/decisions-2026-07-16.md` → Passwords, revised 2026-07-19. Remaining hardening tracked in `docs/pre-events-punchlist.md`.
