...
- [ ] **#0 (PRE-DEPLOY) Confirm the production DB pool role has `BYPASSRLS`.** The webhook lanes resolve connections/email-logs cross-org via `…AnyOrg` resolvers (plain `db.select()`, no org GUC) because a webhook arrives with no org context. In prod this relies on the base pool role bypassing FORCE RLS. **If the prod role is NOT `BYPASSRLS`, every inbound delivery/reply/grant event silently returns 0 rows and is dropped** — both hard gates would fail mysteriously. Verify BEFORE the deploy (final-review ⚠️). Also covers the A.16 grant-hash backfill write. Check: `SELECT rolbypassrls FROM pg_roles WHERE rolname = current_user;` on the prod connection → must be `t`.
- [ ] **#4 (PRE-MULTI-USER) REVOKED-MEMBER LIVE TEST — T1.4 verification. BLOCKS deployment to ANY users beyond Mike / K&K.** The fail-closed-on-revoked-membership fix (multi-tenant remediation T1.4) is proven at the UNIT level (`resolveActiveOrg` logic + mocked-session tests), **NOT** by a live browser reproduction. Before deploying to any external users, **live-test in the browser**: a user removed from an org whose session still points at that org must (a) **NOT** be able to read that org's data, AND (b) **NOT** hit a redirect loop / lockout — they should be cleanly switched (to another org they belong to) or cleared (to onboarding). **Verify the actual click-through, not just the logic.** **(HARD GATE — blocks multi-user deployment.)**
