...
- **Multi-tenant email sender** — `RESEND_API_KEY` + `RESEND_FROM_EMAIL` (consumed by `src/lib/email.ts:sendEmail`) are application-global. Acceptable for V1 single-tenant K&K (every invitation / verification / password-reset email goes out from one verified sender domain). When V2 scales to multiple studios on shared infrastructure, decide between two options: **(a) SaaS rebrand** — keep one neutral sender domain (e.g. `mail.pathwaycrm.app`) and brand all transactional emails as Pathway, with the studio's name in the body and reply-to. Lowest cost; requires no per-org config; recipients see a third-party sender. **(b) Per-org custom sender** — add `organization.email_config jsonb` (or a new `org_email_config` table) holding a verified domain + From: address + reply-to, plus a Resend domain-verification handoff flow. Higher complexity (per-org DNS support burden, domain-verification status, sender warmup tracking) but each studio's emails appear to come from their own brand. **Until that decision lands, don't bake the single-sender assumption deeper** — every new transactional-email surface (e.g. future workflow-driven emails, payment-receipt sends) should route through `sendEmail` so the swap is one config change, not N call-site updates.
